...
@T.prim_func
def ptx_cp_async_barrier(A: T.Buffer((32, 128), "float16"), B: T.Buffer((32, 128), "float16")) -> None:
    T.func_attr({"global_symbol": "default_function", "tir.noalias": True})
    bx = T.env_thread("blockIdx.x")
    tx = T.env_thread("threadIdx.x")
    T.launch_thread(bx, 1)
...
def ptx_cp_async_bulk(A: T.Buffer((32, 128), "float16"), B: T.Buffer((32, 128), "float16")) -> None:
    ...
    with T.block():
        ...
        barrier[0] = 0
        T.evaluate(T.ptx_init_barrier_thread_count("barrier", 0, 32, dtype=""))
        T.evaluate(T.ptx_init_barrier_byte_count("barrier", 0, 256, dtype=""))

        T.evaluate(
            T.ptx_cp_async_bulk(
                A_shared.data, tx * 128, A.data, tx * 128, 256, "barrier", 0, dtype="float16"
            )
        )
...
@T.prim_func
def ptx_cp_async_bulk_multithread(A: T.Buffer((32, 128), "int8"), B: T.Buffer((32, 128), "int8")) -> None:
    T.func_attr({"global_symbol": "default_function", "tir.noalias": True})
    bx = T.env_thread("blockIdx.x")
    tx = T.env_thread("threadIdx.x")
    T.launch_thread(bx, 1)
    T.launch_thread(tx, 32)
    with T.block():
        A_shared = T.alloc_buffer([32, 128], "int8", scope="shared")
        barrier = T.alloc_buffer([1], "uint64", scope="shared")
        T.reads(A[0:1, 0:128])
        T.writes(B[0:1, 0:128])
        ...
        T.evaluate(
            T.ptx_cp_async_bulk(
                A_shared.data, tx * 128, A.data, tx * 128, 128, "barrier", 0, dtype="int8"
            )
        )
...
@tvm.testing.requires_cuda_compute_version(8)
def test_ptx_cp_async_bulk_multithread():
    f = ptx_cp_async_bulk_multithread

    mod = tvm.build(f, target="cuda")
    A_np = np.random.rand(32, 128).astype("int8")
    B_np = np.zeros((32, 128)).astype("int8")
    dev = tvm.cuda(0)
    A_nd = tvm.nd.array(A_np, device=dev)
    B_nd = tvm.nd.array(B_np, device=dev)
    mod(A_nd, B_nd)
...
if __name__ == "__main__":
    ...
    test_ptx_cp_async_bulk_multithread()
